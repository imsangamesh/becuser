...
class MyNetFileViewer extends StatelessWidget {
  const MyNetFileViewer(this.url, this.title, {Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(title)),
      body: SfPdfViewer.network(url, pageLayoutMode: PdfPageLayoutMode.single),
    );
  }
}
...
class MyFullScrNetImageViewer extends StatelessWidget {
  const MyFullScrNetImageViewer(this.url, {Key? key}) : super(key: key);
